...
do
	    case "$1" in
		    ...
		    -tagMapFiles) tagMaps="-DtagMaps=true";;
		 	...
		 	-repoCache) repoCache="-DfetchCacheLocation=$2"; shift;;							       
		 	-branch) relengBranch="$2"; shift;;
			-root)	writableBuildRoot="$2"; shift;;
			-noTag) noTag=true;;						              
			-*)		  		 		   		 		  		 		    		 		  		 		   		 		  		 		    		 		  		 		   		 		  		 		    
...
pull() {
        ...
        if [ ! -d $directory ]; then
		git config --add branch.integration.rebase true
                echo git clone $1
                git clone $1
                cd $directory
