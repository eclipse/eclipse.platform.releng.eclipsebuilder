...
latest downloads from the eclipse project</font><br></td>
...
<p>On this page you can find the latest <a href="build_types.html" target="_top">builds</a> produced by 
the <a href="http://www.eclipse.org/eclipse" target="_top">Eclipse Project</a>. To get started run the program and go through the user and developer 
documentation provided in the online help system. If you have problems downloading the drops4, contact the <font size="-1" face="arial,helvetica,geneva"><a href="mailto:webmaster@eclipse.org">webmaster</a></font>. 
...
<p>
<font size="+1"><b>Eclipse 3.x</b> downloads are <a href="http://download.eclipse.org/eclipse/downloads/eclipse3x.php">here</a>.</font><br>Other eclipse.org project downloads are available <A HREF="http://www.eclipse.org/downloads/index.php">here</A>.</p>

<p>
See also <a href="http://archive.eclipse.org/eclipse/downloads/index.php">archived builds</a> (including language packs) and see a list of
<a href="http://wiki.eclipse.org/Eclipse_Project_Update_Sites">p2 update sites</a>.</p>
...
foreach($dropType as $value) {
    ...
    if (array_key_exists($prefix,$buckets)) {
        ...
        // echo "<td><a href=license.php?license=drops4/$fileName>$parts[1]</a></td>";

        // Comment the line below if we need click through licenses.
        if (count($parts)==3)
            echo "<td><a href=\"drops4/$fileName/index.php\">$parts[1]</a></td>";
        if (count($parts)==2)
            echo "<td><a href=\"drops4/$fileName/index.php\">$fileName</a></td>";

        if (array_key_exists($fileName,$timeStamps)) { 
    ...
    if (array_key_exists($prefix,$buckets)) {
        ...
        if (isset($aBucket)) {
            ...
            foreach($aBucket as $innerValue) {
                ...
                // Uncomment the line below if we need click through licenses.
                // echo "<td><a href=\"license.php?license=drops4/$innerValue\">$parts[1]</a></td>";

                // Comment the line below if we need click through licenses.
                if (count ($parts)==3)
                    echo "<td><a href=\"drops4/$innerValue/index.php\">$parts[1]</a></td>";
                if (count ($parts)==2)
                    echo "<td><a href=\"drops4/$innerValue/index.php\">$innerValue</a></td>";
